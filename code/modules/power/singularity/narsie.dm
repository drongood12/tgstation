...
/proc/begin_the_end()
	...
	if(QDELETED(GLOB.cult_narsie)) // uno
		priority_announce("Отчет о состоянии дел? Мы обнаружили аномалию, но она почти сразу исчезла.","Исследовательский Отдел Центрального Командования", 'sound/misc/notice1.ogg')
		GLOB.cult_narsie = null
		sleep(20)
		INVOKE_ASYNC(GLOBAL_PROC, .proc/cult_ending_helper, 2)
		return
	priority_announce("В вашем секторе обнаружено акаузальное пространственное событие. Событие помечено как класс конца света XK. Направление всех доступных ресурсов на моделирование решений. РЕШЕНИЕ БУДЕТ ЧЕРЕЗ: 60 СЕКУНД.","Исследовательский Отдел Центрального Командования", 'sound/misc/airraid.ogg')
	sleep(500)
	if(QDELETED(GLOB.cult_narsie)) // dos
		priority_announce("Моделирование прервано, датчики сообщают, что случайное событие нормализуется. Хорошая работа, команда.","Исследовательский Отдел Центрального Командования", 'sound/misc/notice1.ogg')
		GLOB.cult_narsie = null
		sleep(20)
		INVOKE_ASYNC(GLOBAL_PROC, .proc/cult_ending_helper, 2)
		return
	priority_announce("Завершено моделирование акаузального пространственного события. Дешифровка файла решения сейчас. ВРЕМЯ ДЕШИФРОВКИ: ОДНА МИНУТА.","Исследовательский Отдел Центрального Командования")
	sleep(50)
	...
	if(QDELETED(GLOB.cult_narsie)) // tres
		priority_announce("Обнаружена нормализация пространства! Файл решения удалён!","Исследовательский Отдел Центрального Командования", 'sound/misc/notice1.ogg')
		GLOB.cult_narsie = null
		sleep(20)
