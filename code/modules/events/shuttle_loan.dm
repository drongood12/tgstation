...
/datum/round_event/shuttle_loan/announce(fake)
	...
	switch(dispatch_type)
		if(HIJACK_SYNDIE)
			priority_announce("Карго: Синдикат пытается проникнуть на вашу станцию. Если вы позволите им захватить ваш грузовой шаттл, вы избавите нас от головной боли.","Контрразведка Центрального Командования")
		if(RUSKY_PARTY)
			priority_announce("Карго: Группа разгневанных русских хочет устроить вечеринку. Можете ли вы отправить им свой грузовой шаттл, а затем заставить их исчезнуть?","Горячая Линия Центрального Командования")
		if(SPIDER_GIFT)
			priority_announce("Карго: Клан Пауков прислал нам таинственный подарок. Можем ли мы отправить его вам, чтобы посмотреть, что внутри?","Дипломатический Корпус Центрального Командования")
		if(DEPARTMENT_RESUPPLY)
			priority_announce("Карго: Похоже, в этом месяце мы заказали два ящика вместо одного нашего отдела пополнения запасов. Мы можем послать их вам?","Отдел Снабжения Центрального Командования")
			thanks_msg = "Грузовой шаттл должен вернуться через пять минут."
			bonus_points = 0
		if(ANTIDOTE_NEEDED)
			priority_announce("Карго: Ваша станция была выбрана для эпидемиологического исследовательского проекта. Отправьте нам свой грузовой шаттл, чтобы получить ваши исследовательские образцы.", "Отдел Научных Инициатив Центрального Командования")
		if(PIZZA_DELIVERY)
			priority_announce("Карго: Похоже, что соседняя станция случайно доставила вам свою пиццу вместо этого.", "Отдел Доставки Пиццы Центрального Командования")
			thanks_msg = "Грузовой шаттл должен вернуться через пять минут.."
			bonus_points = 0
		if(ITS_HIP_TO)
			priority_announce("Карго: Один из наших грузовых шаттлов, перевозящих партию пчел, подвергся нападению экотеррористов. Не могли бы вы навести здесь порядок?", "Отдел Уборщиков Центрального Командования")
			bonus_points = 20000 //Toxin bees can be unbeelievably lethal
		if(MY_GOD_JC)
			priority_announce("Карго: Мы обнаружили взрывоопасную бомбу около нашего эксклюзивного шаттла. Если вы чувствуете, что справитесь с задачей, мы заплатим вам за ее обезвреживание.", "Отдел безопасности Центрального Командования")
			thanks_msg = "Бомба отправлена вам вместе с шаттлом снабжения. Рекомендуется эвакуировать карго."
			bonus_points = 45000 //If you mess up, people die and the shuttle gets turned into swiss cheese

/datum/round_event/shuttle_loan/proc/loan_shuttle()
	priority_announce(thanks_msg, "Грузовой шаттл, захваченный Центральным Командованием.")
